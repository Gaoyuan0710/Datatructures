...
/*
void Select( HTNODE *HT, int p, int *s1, int *s2 )
{
	int i;
	...
	for ( i = 1; i <= p; i ++ )
		...
		for ( i = 1; i <= p; i ++ )
				if ( ( HT[i].code.count < HT[*s1].code.count ) && ( HT[i].parent == 0 ) )
							*s1 = i;
			...
			for ( i = 1; i <= p; i ++ )
				...
				for ( i = 1; i <= p; i ++ )
						if ( ( HT[i].code.count < HT[*s2].code.count ) && ( i != *s1 ) && ( HT[i].parent == 0 ) )
									*s2 = i;
}
...
int main(int argc, char *argv[])
{
	...
	head = read_file(head, "/home/gaoyuan/source/3.txt");
...
q = code;
	...
	trans_to_code(code, "/home/gaoyuan/source/3.txt", "/home/gaoyuan/source/from.txt");

	press();
	unpress();

	Get_Language(hs, count, "/home/gaoyuan/source/to.txt", "/home/gaoyuan/source/ffff.txt");
